...
Client Application: A dedicated mobile application or lightweight browser interface launched after scanning the unique QR code of an order. The browser interface will receive one-way status-change signals through Server-Sent Events and restore authoritative state through REST after a temporary connection loss.
...
4. Usage Scenario
The scenario begins when a customer places an order at the checkout, after which they scan a generated QR code using their phone's camera. This action immediately launches a dedicated mobile app, if the customer has one installed, or a lightweight web app in their phone's browser. As an additional feature, an App Clip can be integrated on iOS devices, completely eliminating the need to download and install any software. The customer-visible order label and its initial `IN_PREPARATION` status are displayed on the screen. When the meal is ready, the restaurant employee changes its status – this can be done using a dedicated panel in my system on a tablet or via an external POS system, if the restaurant has opted for such integration via a shared API. This action produces a Server-Sent Events signal, and the browser refreshes the visible state through REST. Once the meal is served, the staff marks the order as completed and the live event stream closes. The tracking link continues to show the terminal state and cannot be used to change or reuse the order. Reliable background alerts, vibration, sound, and installable PWA behavior are later enhancements rather than promises of the initial browser flow.
