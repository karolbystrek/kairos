...
The walking vertical slice is local-only. The Spring API now contains the first staff-authentication increment: provisioned local username/password accounts, signed access and rotating refresh credentials in secure cookies, CSRF protection, logout and revocation, current-account retrieval, account provisioning, tenant/location application authorization, and authenticated order audit identity. The panel route remains a Server Component and renders one interactive staff-panel boundary. Its SWR current-account gate and shared native-`fetch` client add CSRF headers to unsafe requests, serialize authentication-cookie mutations with a browser Web Lock when available, recheck the current session after acquiring that lock, and perform at most one refresh and request retry after `401`. Authentication credentials remain unavailable to JavaScript. Treat one browser profile as one signed-in account: same-account tabs are best-effort, immediate cross-tab UI synchronization is not required, and different accounts in different tabs are unsupported. The next panel increment is capability-gated manager/operator account provisioning for existing tenants and locations. Tenant registration and public self-registration remain excluded; the first tenant, location, and administrator are bootstrapped out of band until a separate onboarding contract is designed. OAuth2/OIDC login, PostgreSQL RLS, WebSocket delivery, an OpenAPI endpoint, and generated REST clients remain deferred. Anonymous customer REST tracking remains available through SWR-managed client state backed by handwritten native `fetch` requests. Production migrations must still create an empty, deployable schema rather than demo data.
