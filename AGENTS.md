...
* Spring Boot is the system of record and the sole owner of business rules, authentication, authorization, tenant isolation, persistence, SSE delivery, External Integration access, webhooks, and outbox processing. Application-level request rate limiting is deferred to a future API gateway in front of Spring; do not add an in-process rate limiter to the current local slice.
